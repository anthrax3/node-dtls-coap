...
//dtls.setLogLevel( dtls.logLevel.FINEST );
...
Agent.prototype._init = function initSock(socket) {
  if (this._sock) { //&& typeof(this._opts.dtls) == "undefined") {
    return
  }
  var that = this
  if (typeof(this._opts.dtls) != "undefined") {
     isdtls = true;
     //console.log(this._opts.dtls)
     dtls_endpoint = this._opts.dtls.address
     dtls_endpointPort = this._opts.dtls.port
     dtls_listen_port = this._opts.dtls.dtlsPort
     udp_listen_port = this._opts.dtls.udpPort
     udp = dtls_proxy.createUDPServer(udp_listen_port,"udp4")
     dtls_server = dtls_proxy.createDTLSServer(this._opts.dtls.crt,this._opts.dtls.key,dtls_listen_port,"udp4")
...
Agent.prototype.request = function request(url) {
  this._init()
  //if (typeof(this._opts.dtls) != "undefined" && this._sock){
    //console.log(this)
  //}
  if (!url){
  }
