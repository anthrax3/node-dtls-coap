...
var dgram           = require('dgram')
  ...
  , dtls_proxy      = require('node-dtls-proxy')
  , fs              = require('fs')
  , util            = require('util')
...
CoAPServer.prototype.listen = function(port, address, key, crt, done) {
  ...
  if (key && crt) {
    ...
    dtls_server = dtls_proxy.createDTLSServer(crt,key,dtls_listen_port,"udp4")
    ...
    udp.on('message', function (msg, rinfo) {
      console.log("Encrypting CoAP response from:",[rinfo.address,rinfo.port].join(":"), "and forwarding to UDP2DTLS endpoint");
   ...
   dtls_server.on( 'secureConnection', function( socket ) {
    ...
    socket.on( 'message', function( message ) {
      dtls_socket = socket
      console.log("Decrypting DTLS message from:", [socket.rinfo.address,socket.rinfo.port].join(":"), "and forwarding to CoAP endpoint:");
...
CoAPServer.prototype._handle = function(packet, rsinfo) {
  ...
  response._addCacheEntry = function(key, payload) {
    ...
    block2cache.key = key;
    block2cache.payload = payload;
  }

  //return cached value for blockwise requests
  var cachedResponseSend = false;
  if (packet.token && packet.token.length > 0) {
    debug('check if packet token is in cache');
    if (this._block2Cache.key === packet.token.toString('hex')) {
      debug('found cached payload');
      response.end(this._block2Cache.payload);
...
OutMessage.prototype.end= function(payload) {
  ...
  if (this._request.token && this._request.token.length > 0) {
    this._addCacheEntry(this._request.token.toString('hex'), payload)
  }
